...
const AppBranding: React.FC<AppBrandingProps> = ({
  title,
  description,
  customStyles = "relative w-full h-[80vh] lg:h-[100vh]",
  bannerUrl,
  link,
}) => {
  return (
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full bg-black bg-opacity-30 text-white text-center p-5">
        {title ? (
          ...
          <h1 className="text-xl lg:text-6xl font-semibold mb-4 lg:text-[109px]">
            <span className=" text-green-800">Empowering Communities,</span>{" "}
            <span className="">Simplified.</span>
          </h1>
        )}
        {description ? (
          <p className="text-lg lg:text-2xl mb-6 lg:w-full lg:text-[36px]">
            {description}
          </p>
        ) : (
          <p className="text-lg lg:text-2xl mb-6  lg:text-[28px]">
            Together, we can make a real difference—your investment brings hope
            and progress to millions.
          </p>
        )}
        ...
        {link ? (
          <div className="flex space-x-4">
            ...
            <Link href={`${link}`}>
              ...
              <button className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-md">
                Learn More
              </button>
            </Link>
          </div>
